...
Per-workspace bridge that lets LLM agents (Claude Code, OpenCode, etc.) query local databases directly via HTTP — without manual connection setup. The agent reads `claude.local.md` (auto-generated by the tile) to discover what databases are available and how to call them.
...
**Tile UI:** `Enabled` checkbox (controls `claude.local.md` generation), list of selected databases with RW/RO toggle, list of all discovered databases with add button. Tile reacts to `DatabaseServiceManager.StateChanged` and `SettingsChanged`.
...
**Workspace config:** `.mterminal/databases.json` — `WorkspaceDatabaseTileConfig` with `Enabled` (bool) and `Databases` (list). On change, `claude.local.md` is generated (only when `Enabled = true`).
